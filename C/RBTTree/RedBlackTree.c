...
{
    RBT_InsertNodeHelper(Tree,NewNode); //이진 탐색트리의 노드 삽입 

    NewNode->Color = RED;
    NewNode->Left = Nil;
    NewNode->Right = Nil;

    RBT_RebuildAfterInsert(Tree,NewNode);//규칙바로잡기
}
...
void RBT_RotateRight(RBTNode** Root,RBTNode* Parent)
{
    ...
    Parent->Left = LeftChild->Right; //왼쪽 자식 노드의 오른쪽 자식 노드를 ㅇ부모 노드의 왼쪽 자식으로 등록
    ...
    if(Parent->Parent == NULL) //이 노드가 Root인 경우
        (*Root) = LeftChild;
...
{
    while(X != (*Root) && X->Parent->Color == RED)//4번 규칙을 위반하고 있는 동안
    {
    ...
    (*Root)->Color = BLACK;//root node is must be black.
}
...
void RBT_RebuildAfterRemove(RBTNode** Root,RBTNode* Successor)//Successor는 이중흑색 노드임.
{
    RBTNode* Sibling = NULL;
    
    while(Successor->Parent != NULL && Successor->Color == BLACK)//루트노드거나 빨간색 노드한테 검은색이 넘어가면 루프 종료
    {
