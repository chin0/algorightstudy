...
WINDOW **file_menu_list(int start_col)
{
    ...
    items[0]=newwin(6,19,1,start_col); //새 윈도우 생성
    wbkgd(items[0], COLOR_PAIR(2));
    box(items[0],ACS_VLINE,ACS_HLINE);

    items[1]=subwin(items[0],1,17,2,start_col+1); //items[0]의 서브 윈도우 생성
    items[2]=subwin(items[0],1,17,2,start_col+1);
    items[3]=subwin(items[0],1,17,2,start_col+1);
    items[4]=subwin(items[0],1,17,2,start_col+1);
    for(i = 1; i < 5; i++)
        wprintw(items[i],"%s",file_item[i]);
    wbkgd(items[1], COLOR_PAIR(1)); //배경색
    wrefresh(items[0]);
    return items;
}
...
void about_window()
{
    ...
    wmove(about,1,2);
    wprintw(about, "Ncurses Test Version 0.1");
    wmove(about,2,2);
    wprintw(about, "Make : kafuuchin0");
    wmove(about,3,2);
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if(key == 'a')
        {
            ...
            touchwin(stdscr); //touchwin은 윈도우를 다시 그리는데 사용
            refresh();
        }
        if(key == 'h')
        {
